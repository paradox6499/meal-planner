...
describe("resolveIngredientPricesWithCache", () => {
  ...
  it("свежая запись в кэше — не ходит в сеть вообще", async () => {
    ...
    expect(result.get("Лук")).toEqual({ matched: true, price: 58, productUnit: "кг", xmlId: "1" });
  ...
  it("нет в кэше — идёт живьём и сохраняет результат для следующего раза", async () => {
    ...
    expect(result.get("Гречка")).toEqual({ matched: true, price: 95, productUnit: "кг", xmlId: "9" });
    ...
    expect(second.get("Гречка")).toEqual({ matched: true, price: 95, productUnit: "кг", xmlId: "9" });
    expect(fetch).not.toHaveBeenCalled();
  });
  ...
  it("товар не найден в каталоге — кэширует matched:false тоже", async () => {
    ...
    expect(result.get("Несуществующий ингредиент")).toEqual({ matched: false, price: null, productUnit: null, xmlId: null });
  ...
  it("устаревшая matched-запись (старше MATCHED_TTL_MS) — переспрашивает живьём", async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(result.get("Молоко")).toEqual({ matched: true, price: 90, productUnit: "л", xmlId: "new" });
  });
  ...
  it("живой запрос не удался после исчерпания ретраев — не кэширует провал, отдаёт устаревшую запись, если она была", async () => {
    ...
    expect(result.get("Сыр")).toEqual({ matched: true, price: 500, productUnit: "кг", xmlId: "old-sыр" });
  ...
  it("живой запрос не удался и в кэше вообще ничего не было — matched:false, без записи в кэш", async () => {
    ...
    expect(result.get("Совсем новый ингредиент")).toEqual({ matched: false, price: null, productUnit: null, xmlId: null });
