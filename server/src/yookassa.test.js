...
describe("createPayment", () => {
  ...
  it("шлёт корректный запрос с Basic-авторизацией и Idempotence-Key", async () => {
    ...
    const result = await createPayment(CREDS, {
      amountRub: 299, description: "Съедим Pro — 30 дней", returnUrl: "https://t.me/s_edim_bot", telegramUserId: 42, idempotenceKey: "idem-1",
    });
  ...
  it("бросает читаемую ошибку при отказе ЮKassa", async () => {
    ...
    await expect(
      createPayment(CREDS, { amountRub: 0, description: "x", returnUrl: "x", telegramUserId: 1, idempotenceKey: "x" })
