...
function normalizeVkusvillRecipe(raw, category) {
  ...
  return {
    ...
    devices: [],
    category,
    // null, не 0 — "цена ещё не посчитана" не должно выглядеть как "бесплатно".
    // attachRealCosts проставит настоящее число, если наберётся достаточно
    // совпадений по ингредиентам (см. её комментарий); если нет — так и
    // останется null, и это осознанно, см. комментарий там же.
    cost: null,
...
export async function attachRealCosts(pools) {
  ...
  Object.values(pools).forEach((recipes) => {
    recipes.forEach((recipe) => {
      ...
      // total>0 (хотя бы ОДИН ингредиент нашёлся) не считается достаточным,
      // чтобы выставить recipe.cost всему блюду. Жалоба в чате: "куриная
      // печень с черносливом за 16 ₽" — это блюдо буквально из 2
      ...
      //
      // При недостаточном совпадении recipe.cost НЕ трогаем — он остаётся
      // на дефолтном null (см. normalizeVkusvillRecipe), а не 0. Раньше тут
      // стоял именно 0 — и именно это стало причиной куда более серьёзного
      // прод-бага (жалоба в чате: "план на 15 000 ₽, а все блюда и итог по
      // 0 ₽"): effectiveRecipeCost в data/recipes.js для ВкусВилл-рецептов
      // ВСЕГДА берёт recipe.cost/isRealPrice как есть (isRealPrice у них не
      // undefined, в отличие от статических рецептов) — блюдо с недостаточным
      // совпадением молча выглядело БЕСПЛАТНЫМ вместо "цена неизвестна",
      // и всё это тихо суммировалось в итог. null — честный сигнал "нет
      // данных", buildPlanView/ResultView показывают его как "—", а не как 0.
